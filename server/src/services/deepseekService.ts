 ...
 * @returns 生成的HTML内容
 */
export const generateHtmlWithDeepseek = async (prompt: string, theme?: string): Promise<string> => {
  try {
    // 构建系统提示词
 ...
 * @param theme 可选的主题名称
 */
export const generateHtmlWithDeepseekStreaming = async (prompt: string, res: Response, theme?: string): Promise<void> => {
  try {
    // 构建系统提示词 (与非流式API相同)
    let systemPrompt = `你是一名专业的网页设计师和前端开发专家，对现代 Web 设计趋势和最佳实践有深入理解，尤其擅长创造具有极高审美价值的用户界面。你的设计作品不仅功能完备，而且在视觉上令人惊叹，能够给用户带来强烈的"Aha-moment"体验。
